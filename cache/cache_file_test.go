...
func TestFileFileWriteReadHeader(t *testing.T) {
...
func TestFilePendingEntries(t *testing.T) {
	cfg := config.Cache{
		Name:      "foobar",
		Dir:       testDir,
		MaxSize:   1e6,
...
func TestFileCacheRollback(t *testing.T) {
	c := newFileTestCache(t)
	defer c.Close()
...
func TestFileCacheClean(t *testing.T) {
	cfg := config.Cache{
		Name:    "foobar",
		Dir:     testDir,
		MaxSize: 8192,
...
type testFileResponseWriter struct {
	h http.Header
	b []byte
}

func (trw *testFileResponseWriter) Write(p []byte) (int, error) {
	trw.b = append(trw.b, p...)
	return len(p), nil
}

func (trw *testFileResponseWriter) Header() http.Header {
	if trw.h == nil {
...
func (trw *testFileResponseWriter) WriteHeader(statusCode int) {}
