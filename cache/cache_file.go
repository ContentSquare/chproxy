...
// FileCache represents a file cache.
...
func NewFileCache(cfg config.Cache) (*FileCache, error) {
	if len(cfg.Dir) == 0 {
...
func (c *FileCache) WriteTo(rw http.ResponseWriter, key *Key) error {
	return c.writeTo(rw, key, http.StatusOK)
}

func (c *FileCache) writeTo(rw http.ResponseWriter, key *Key, statusCode int) error {
	f, err := c.get(key)
	if err != nil {
		return err
	}
...
func (c *FileCache) get(key *Key) (*os.File, error) {
	fp := c.filepath(key)
...
func (c *FileCache) registerPendingEntry(path string) bool {
	if c.graceTime <= 0 {
		return true
	}
...
func (c *FileCache) unregisterPendingEntry(path string) {
	if c.graceTime <= 0 {
		return
	}
...
func (c *FileCache) pendingEntriesCleaner() {
	if c.graceTime <= 0 {
		return
	}
...
func (c *FileCache) fileInfoPath(fi os.FileInfo) string {
...
func (c *FileCache) NewResponseWriter(rw http.ResponseWriter, key *Key) (ResponseWriter, error) {
...
func (c *FileCache) MaxSize() uint64 {
	return c.maxSize
}
