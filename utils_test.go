...
func TestSkipLogComment(t *testing.T) {
	type test struct {
		name     string
		query    string
		expected string
	...
	tests := []test{
		{
			name:     "simple removal of log_comment",
			query:    "SELECT number FROM numbers(10) SETTINGS log_comment = 'foo ' bar'",
			expected: "SELECT number FROM numbers(10) SETTINGS ",
		},
		{
			name: "subquery with log_comment",
			query: `SELECT number
			FROM
			(
				SELECT number
				FROM numbers(10)
				SETTINGS optimize_read_in_order=1, cast_keep_nullable=1, log_comment = 'foo \' bar', optimize_read_in_order=1
			)
			WHERE (number % 2) = 0
			HAVING number > 5
			ORDER BY number DESC
			SETTINGS log_comment = 'foo \' baz'
			FORMAT CSV`,
			expected: `SELECT number
			FROM
			(
				SELECT number
				FROM numbers(10)
				SETTINGS optimize_read_in_order=1, cast_keep_nullable=1, optimize_read_in_order=1
			)
			WHERE (number % 2) = 0
			HAVING number > 5
			ORDER BY number DESC
		...
		{
			name: "query with log_comment field",
			query: `SELECT 'total' as name, count() as total
			FROM system.query_log
			SETTINGS distributed_group_by_no_merge=0
			UNION ALL
			SELECT 'foo' as name, countIf(log_comment = 'foo')
			FROM system.query_log
			SETTINGS distributed_group_by_no_merge=1`,
			expected: `SELECT 'total' as name, count() as total
			FROM system.query_log
			SETTINGS distributed_group_by_no_merge=0
			UNION ALL
			SELECT 'foo' as name, countIf(log_comment = 'foo')
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			testSkipLogComment(t, tt.query, tt.expected)
		})
	}
}

func testSkipLogComment(t *testing.T, q, expectedQ string) {
	t.Helper()
	s := skipLogComment([]byte(q))
	if string(s) != expectedQ {
		t.Fatalf("unexpected result %s; expecting %s", s, expectedQ)
	}
}
